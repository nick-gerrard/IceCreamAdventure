...
local function buildEntities()
	...
	for _, spawn in ipairs(level:getSpawns()) do
		...
		elseif spawn.type == "pigeon" then
			table.insert(entities, Pigeon.new(spawn.x, spawn.y))
...
function love.draw()
	...
	love.graphics.print("Time: " .. Timer.display(), 100, 190)
	if GameState.won then
		...
		love.graphics.printf(
			GameState.score .. "/" .. GameState.totalIceCream .. " ice creams collected",
			0,
			C.VIRTUAL_H / 2 + 10,
			C.VIRTUAL_W,
			"center"
